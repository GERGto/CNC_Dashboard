...
def _has_automatic_interval(task):
    ...
    if isinstance(interval_value, str) and interval_value.strip() == "-":
        return False
    return interval_type in {"runtimeHours", "calendarMonths"}


def is_maintenance_task_due(task, spindle_runtime_sec, now=None):
...
class MachineStatusService:
    ...
    def build_snapshot(self, spindle_runtime_sec, maintenance_tasks, relay_board):
    ...
    def _collect_due_task_ids(self, maintenance_tasks, spindle_runtime_sec):
        due_task_ids = []
        for task in maintenance_tasks if isinstance(maintenance_tasks, list) else []:
            ...
            if not task_id:
                continue
            if is_maintenance_task_due(task, spindle_runtime_sec):
